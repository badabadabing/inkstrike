...
function onKey(c) {
  ...
  if (!pl.alive) return;
  if (c === 'Digit1' && pl.inv[1]) switchTo(pl.inv[1]); else if (c === 'Digit2' && pl.inv[2]) switchTo(pl.inv[2]); else if (c === 'Digit3') switchTo('knife'); else if (c === 'Digit4') { const l = nadeList(pl); if (l.length) switchTo(l[(l.indexOf(pl.cur) + 1) % l.length]); } else if (c === 'KeyX' && G.mode === 'range') { FX.clearDecals(); SFX.ui(); }
...
const NADES = ['he', 'flash', 'smoke'], nadeList = pl => NADES.filter(k => pl.nades[k] > 0), weaponOrder = pl => [pl.inv[1], pl.inv[2], 'knife', ...nadeList(pl)].filter(Boolean);
const hasWeapon = k => { const pl = G.player; return k === 'knife' || pl.inv[1] === k || pl.inv[2] === k || (WEAPONS[k].nade && pl.nades[k] > 0); };

/* ---------------- player ---------------- */
function makePlayer(team) { const p = makeEnt(team, '你', true); Object.assign(p, { inv: { 1: null, 2: 'p9' }, ammo: {}, cur: 'p9', last: 'knife', nades: { he: 0, flash: 0, smoke: 0 }, landSlow: 0, act: null, nextFire: 0, reloadEnd: -1, reloadStart: 0, drawEnd: 0, shots: 0, lastShot: -9, recP: 0, recY: 0, recTP: 0, recTY: 0, punch: 0, roll: 0, scoped: 0, bobPhase: 0, pending: null, deathT: 0 }); return p; }
function giveWeapon(pl, k) { const w = WEAPONS[k]; if (w.nade) { pl.nades[k] = 1; return; } pl.inv[w.slot] = k; pl.ammo[k] = { mag: w.mag, res: w.res }; }
...
function updatePlayer(dt) {
  ...
  if (lad && (lup !== 0 || pl.pos.y > lad.y1 + .15) && !(lup < 0 && pl.onGround)) { if (k.Space) { pl.ladCd = now + .45; pl.vel.y = 3.5; pl.vel.x += Math.sin(pl.yaw) * 2.5; pl.vel.z += Math.cos(pl.yaw) * 2.5; } else { pl.onGround = false; pl.vel.y = (lup > 0 && pl.pos.y >= lad.y2 ? 0 : lup * 3.4) + GRAV * dt; if (pl.pos.y < lad.y2 - .1) { pl.vel.x *= .7; pl.vel.z *= .7; } pl.stepAcc += Math.abs(lup) * 3.4 * dt; if (pl.stepAcc > 1.1) { pl.stepAcc = 0; SFX.step(null, .2, 'metal'); } } }
  ...
  if (pl.landed > 2.6) { pl.landSlow = .3; pl.vel.x *= .6; pl.vel.z *= .6; }   // landing penalty: kills endless bunny-hopping
  ...
  const moved = Math.hypot(pl.pos.x - px, pl.pos.z - pz); if (pl.onGround) { pl.bobPhase += moved * 2.0; if (hs > 3 && !walking) { pl.stepAcc += moved; if (pl.stepAcc > 2.1) { pl.stepAcc = 0; SFX.step(null, .22, surfaceAt(pl.pos)); botHear(pl.pos, pl.team, 11); } } }
  // ---- weapon ----
  ...
  if (ready && now >= pl.nextFire) {
    ...
    else if (w.nade) { const kind = pl.cur; if (G.fireEdge && pl.nades[kind] > 0) { pl.nextFire = now + 1; VM.atk = 0; SFX.swish(); pl.pending = { t: now + .2, fn: () => { throwNade(pl, kind); pl.nades[kind]--; if (G.mode === 'range') pl.nades[kind] = 1; if (pl.nades[kind] <= 0) switchTo(nadeList(pl)[0] || pl.inv[1] || pl.inv[2]); } }; } }
...
function throwNade(e, kind = 'he') { const cp = Math.cos(e.pitch), dx = -Math.sin(e.yaw) * cp, dy = Math.sin(e.pitch), dz = -Math.cos(e.yaw) * cp; if (!G.nadeFm) { G.nadeFm = fillMat({ objSpace: true, freq: 60 }); }
...
function kill(e, by, wkey, head, dir) {
  ...
  addFeed(by, e, wkey, head); PROG.onKill(e, by, wkey); e.act = null;
  if (e.isPlayer) { e.scoped = 0; G.deathCam = { t: 0, yaw: e.yaw, by }; $('dead').classList.add('on'); $('deadBy').textContent = by && by !== e ? `被 ${by.name} 用 ${WEAPONS[wkey] ? WEAPONS[wkey].name : '环境'} 击倒` : '你倒下了'; if (G.mode === 'comp') { e.inv[1] = null; e.inv[2] = 'p9'; e.nades = { he: 0, flash: 0, smoke: 0 }; e.armor = 0; e.helmet = false; } closeBuy(); PROG.report('本次交战'); }
  else { const f = dir ? (-Math.sin(e.yaw) * dir.x - Math.cos(e.yaw) * dir.z) : -1; e.fallDir = f > 0 ? -1 : 1; e.target = null; e.model.mark.visible = false; }
  if (by && by.isPlayer && e !== by) { SFX.kill(); banner(head ? '爆头击倒' : '击倒', G.mode === 'comp' && WEAPONS[wkey] ? `${e.name}  +$${WEAPONS[wkey].reward}` : e.name); }
...
function botBuy(b) {
  if (b.dummy) { b.armor = b.rangeI % 2 ? 100 : 0; b.helmet = b.rangeI % 2 === 1; setEntWeapon(b, b.rangeI % 2 ? 'm4' : 'ak'); return; }
  b.nade = null; if (G.mode === 'dm' ? Math.random() < .3 : (b.money >= 2500 && Math.random() < .55)) { b.nade = pick(['he', 'flash', 'flash', 'smoke']); if (G.mode === 'comp') b.money -= WEAPONS[b.nade].price; }
...
function startRound() {
  ...
  else banner('死斗模式', `率先 ${DM_KILLS} 次击倒的队伍获胜 · 随时可采购换装`); SFX.ui(660);
}
function checkRoundEnd() { const a = { red: 0, blue: 0 }; for (const e of G.ents) if (e.alive) a[e.team]++; if (!a.blue) endRound('red', '防守方全员阵亡'); else if (!a.red && BOMB.state !== 'planted') endRound('blue', '进攻方全员阵亡'); }
function endRound(win, reason) {
  ...
  const mine = win === G.team; const mvp = PROG.mvp(win); banner(mine ? '回合胜利' : '回合失利', `${reason || (win === 'blue' ? '蓝方拿下本回合' : '红方拿下本回合')}${mvp ? ' · ' + mvp : ''}`, mine ? 'win' : 'lose'); SFX.bell(mine); closeBuy(); PROG.report('回合伤害统计');
}
...
function updateFlow(dt) {
  ...
  else if (G.state === 'live') { if (G.mode === 'comp') { if (G.timer <= 0 && BOMB.state !== 'planted') endRound('blue', '时间耗尽'); }
    else if (G.mode === 'range') { const pl = G.player; pl.hp = 100; for (const k in pl.ammo) pl.ammo[k].res = WEAPONS[k].res; for (const e of G.bots) if (!e.alive && G.now >= e.respawnT - 1.2) spawnEnt(e); }
...
function drawBoard() { const row = e => `<tr class="${e.isPlayer ? 'me' : ''} ${e.alive ? '' : 'out'}"><td>${e.name}${e.mvps ? ' ★' + e.mvps : ''}${BOMB.carrier === e && e.team === G.team ? ' ◆' : ''}</td><td>${e.kills}</td><td>${e.deaths}</td><td>${G.mode === 'comp' && e.team === G.team ? '$' + e.money : ''}</td></tr>`;
...
function drawFx(dt) {
  ...
  for (const q of FX.nums) { _cv.set(q.x, q.y, q.z).applyMatrix4(camera.matrixWorldInverse); if (_cv.z > -.3) continue; _cv.set(q.x, q.y, q.z).project(camera); g.globalAlpha = 1 - q.t / .9; g.fillStyle = q.head ? '#d42a2a' : '#16161c'; g.fillText((q.head ? '◎ ' : '') + q.val, (_cv.x * .5 + .5) * W, (-_cv.y * .5 + .5) * H); }
  g.font = '600 12px "IBM Plex Mono", monospace'; for (const e of G.ents) { if (e === pl || !e.alive || e.team !== pl.team) continue; _cv.set(e.pos.x, e.pos.y + 2.05, e.pos.z).applyMatrix4(camera.matrixWorldInverse); if (_cv.z > -.5) continue; _cv.set(e.pos.x, e.pos.y + 2.05, e.pos.z).project(camera); if (Math.abs(_cv.x) > 1.1 || Math.abs(_cv.y) > 1.1) continue; const sx = (_cv.x * .5 + .5) * W, sy = (-_cv.y * .5 + .5) * H, col = pl.team === 'red' ? '#d42a2a' : '#2d6cb3'; g.globalAlpha = .9; g.fillStyle = 'rgba(245,242,234,.8)'; const tw = g.measureText(e.name).width + 10; g.fillRect(sx - tw / 2, sy - 9, tw, 16); g.fillStyle = col; g.fillText(e.name, sx, sy + 3); }
  g.globalAlpha = 1; if (!pl.alive || G.state === 'menu') return;
  if (!pl.scoped && !WEAPONS[pl.cur].scope) { const XC = { ink: '#16161c', red: '#d42a2a', blue: '#2d6cb3', amber: '#e9a520', green: '#2e9e4a' }[G.set.xh] || '#16161c', sz = G.set.xhSize, gap = (G.set.xhStatic ? 5 : 4 + (pl.spread || 0) * 900) * sz, L = 8 * sz; g.lineCap = 'butt'; for (const [c, lw] of [['#f5f2ea', 5 * sz], [XC, 2.4 * sz]]) { g.strokeStyle = c; g.lineWidth = lw; g.beginPath(); g.moveTo(cx - gap - L, cy); g.lineTo(cx - gap, cy); g.moveTo(cx + gap, cy); g.lineTo(cx + gap + L, cy); g.moveTo(cx, cy - gap - L); g.lineTo(cx, cy - gap); g.moveTo(cx, cy + gap); g.lineTo(cx, cy + gap + L); g.stroke(); } g.fillStyle = '#f5f2ea'; g.fillRect(cx - 2, cy - 2, 4, 4); g.fillStyle = XC; g.fillRect(cx - 1, cy - 1, 2, 2); }
...
function drawRadar() {
  ...
  if (G.mode === 'comp' && (BOMB.state === 'planted' || (BOMB.state === 'dropped' && pl.team === 'red'))) { const bx = (BOMB.pos.x - ref.pos.x) * k, bz = (BOMB.pos.z - ref.pos.z) * k; g.fillStyle = '#e9a520'; g.strokeStyle = '#16161c'; g.lineWidth = 1.5; g.beginPath(); g.moveTo(bx, bz - 6); g.lineTo(bx + 6, bz); g.lineTo(bx, bz + 6); g.lineTo(bx - 6, bz); g.closePath(); g.fill(); g.stroke(); }
